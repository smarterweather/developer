...
| [`@smarterweather/mcp-onboarding`](./mcp-onboarding) | stdio bridge to the hosted developer-onboarding MCP server at `mcp.developers.smarterweather.com`. Anonymous discovery by default; local `start_trial` writes `.env` and never prints the bearer; set `SMARTERWEATHER_ONBOARDING_AUTH=required` for OAuth. | GA on `latest` |
